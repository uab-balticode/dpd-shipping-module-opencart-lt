...
//            'Po_type' => $this->config->get('senddata_service'),
	...
	foreach ($orderList as $orderItem)
	{
		...
		if (strpos($orderObj['shipping_code'], self::PARCELSTORECODE) !== FALSE)
		{
			...
			$errorText .= $this->language->get('text_bad_order_with_id').': '.$orderItem.', '.$this->language->get('text_error').': parcelshop not found: '.$orderObj['shipping_method'].'; ';
		    }
		}
		else if (strpos($orderObj['shipping_code'], self::COURIERCODE) !== FALSE)
		{
			...
			$errorText .= $this->language->get('text_bad_order_with_id').': '.$orderItem.', '.$this->language->get('text_error').': '.$this->language->get('text_not_dpd_shipping_method');
		}
		if ($returnDetails)
		{
			...
			else if ($requestResponse&&isset($requestResponse['status'])&&$requestResponse['status']=="err")
			{
				...
				$this->model_sale_order->addOrderHistory($orderItem, array('order_status_id' => $orderObj['order_status_id'], 'comment' => $this->language->get('text_error').': '.print_r($requestResponse, true), 'notify' => 0, 'no_send' => 1));
				if ($requestResponse['errlog'])
				{
					$errorText .= $this->language->get('text_bad_order_with_id').': '.$orderItem.', '.$this->language->get('text_error').': '.$requestResponse['errlog'].'; ';
				}
